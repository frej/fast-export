...
def export_tags(ui,repo,old_marks,mapping_cache,count,authors,tagsmap,tag_encoding):
  l=repo.tagslist()
  for tag,node in l:
    tag=force_utf8(tag,tag_encoding)
...
def verify_heads(ui,repo,cache,force,ignore_unnamed_heads,branchesmap,branch_encoding):
  branches={}
  ...
  for _,_,b in l:
    b=force_utf8(b,branch_encoding)
    b=get_branch(b)
...
def hg2git(repourl,m,marksfile,mappingfile,headsfile,tipfile,
           ...
           sob=False,force=False,ignore_unnamed_heads=False,hgtags=False,notes=False,encoding='',fn_encoding='',tag_encoding='',branch_encoding='',
           plugins={}):
  ...
  if not verify_heads(ui,repo,heads_cache,force,ignore_unnamed_heads,branchesmap,branch_encoding):
    return 1
