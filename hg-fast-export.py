...
# this dict should be pickled for future resume
attribute_file = {}
...
def build_lfs_attributes(branch):
  sys.stderr.write("Generating gitattribute for branch %s\n" % (branch))
  sys.stderr.write("content of attribute_file %s\n" %(str(attribute_file)))
  if branch in attribute_file:
    content = "# generated by hg-fast-export\n"
    for entry in sorted(attribute_file[branch]):
      content += escape_space(entry) + " filter=lfs diff=lfs merge=lfs -text\n"
    return content
  return None

def remove_from_lfs(branch, fileToRemove):
  if branch in attribute_file:
    if fileToRemove in attribute_file[branch]:
      attribute_file[branch].remove(fileToRemove)

def add_to_lfs(branch, fileToAdd):
  if branch not in attribute_file:
    attribute_file[branch] = sets.Set()
  if fileToAdd not in attribute_file[branch]:
    attribute_file[branch].add(fileToAdd)

def merge_lfs_attributes(branch, merged_branch):
  if merged_branch in attribute_file:
    sys.stderr.write("Merging %s into %s\n" % ( merged_branch, branch))
    if branch not in attribute_file:
...
def export_commit(ui,repo,revision,old_marks,max,count,authors,
  ...
  if len(parents) == 0:
    ...
    if len(parents) == 1:
      ...
      type='thorough delta'
      # in case we have added a gitattribute generated file
      # we need to copy the merged branch
      merged_branch = get_branchname(repo.changectx(parents[1]).branch())
      merge_lfs_attributes(branch, merged_branch)
  ...
  for entry in added + changed:
    if len(entry) > 5 and entry[:5] == ".hglf":
      add_to_lfs(branch, strip_leading_slash(entry[5:]))
      new_lfs_entry = True
  for entry in removed:
    if len(entry) > 5 and entry[:5] == ".hglf":
      remove_from_lfs(branch, strip_leading_slash(entry[5:]))
      new_lfs_entry = True
  ...
  if new_lfs_entry:
    ...
    if gitAttributes is not None:
      wr('M %s inline %s' % ("100644", ".gitattributes"))
