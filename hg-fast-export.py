...
from hg2git import setup_repo,fixup_user,get_branch,get_changeset,change_encoding
...
def export_file_contents(ctx,manifest,files,hgtags,encoding='',plugins={}):
  ...
  for file in files:
    ...
    if not hgtags and file == b".hgtags":
      ...
      continue
    filename=change_encoding(file,encoding)
...
def export_commit(ui,repo,revision,old_marks,max,count,authors,
  ...
  for filename in removed:
    filename=change_encoding(filename,fn_encoding)
...
def export_tags(ui,repo,old_marks,mapping_cache,count,authors,tagsmap,tag_encoding):
  l=repo.tagslist()
  for tag,node in l:
    tag=change_encoding(tag,tag_encoding)
