    ...
    if submodule_mappings and ctx.substate and file==".hgsubstate":
      ...
      for name in ctx.substate:
        ...
        if not name in subrepo_cache:
          subrepo_cache[name]=(load_cache(gitRepoLocation+"/hg2git-mapping"), load_cache(gitRepoLocation+"/hg2git-marks", lambda s: int(s)-1))
...
if __name__=='__main__':
  ...
  if options.subrepo_map:
      if not os.path.exists(options.subrepo_map):
        ...
        sys.exit(1)
      submodule_mappings=load_mapping('subrepo mappings',options.subrepo_map,False)
