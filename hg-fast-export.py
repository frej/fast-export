...
# those 2 dict should be pickled for future resume
attribute_tree = {}
attribute_file = {}
...
def add_to_lfs(branch, fileToAdd):
  if not branch in attribute_tree:
    attribute_tree[branch] = "# generated by hg-fast-export\n"
    attribute_file[branch] = sets.Set()
  if fileToAdd not in attribute_file[branch]:
    attribute_tree[branch] += escape_space(fileToAdd) + " filter=lfs diff=lfs merge=lfs -text\n"
    attribute_file[branch].add(fileToAdd)
  return attribute_tree[branch]
...
def export_file_contents(ctx,manifest,files,hgtags,encoding=''):
  ...
  for file in files:
    ...
    if filename[:5] == ".hglf":
      ...
      if lfsFileCached is not None:
        ...
        gitAttribute = add_to_lfs("allbranches", strip_leading_slash(filename))
      else:
  ...
  if gitAttribute is not None:
    wr('M %s inline %s' % (gitmode(manifest.flags(file)), ".gitattributes"))
    wr('data %d' % len(gitAttribute))
    wr(gitAttribute)
